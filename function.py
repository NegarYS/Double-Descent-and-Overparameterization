...
class Sigmoid(Function):
    ...
    def backward(ctx, upstream_grad): 
        # f'(x) = f(x) * (1 - f(x))
        grad_a = upstream_grad * ctx.output * (1 - ctx.output) if ctx.needs_input_grad[0] else None
        return (grad_a,)
...
class Softmax(Function):
    ...
    def forward(ctx, a, axis):
        a_stable = a.data - np.max(a.data, axis=axis, keepdims=True)
        exp_a = np.exp(a_stable)
        output = exp_a / np.sum(exp_a, axis=axis, keepdims=True)
    ...
    def backward(ctx, upstream_grad):
        ...
        grad_a = output * (upstream_grad - np.sum(upstream_grad * output, axis=axis, keepdims=True)) \
...
class CrossEntropyWithSoftmax(Function):
    ...
    def forward(ctx, y_pred, y_true):
        y_stable = y_pred.data - np.max(y_pred.data, axis=1, keepdims=True)
        exp_y = np.exp(y_stable)
        softmax_output = exp_y / np.sum(exp_y, axis=1, keepdims=True)

        batch_size = y_pred.data.shape[0]
        loss = -np.sum(y_true.data * np.log(softmax_output + 1e-12)) / batch_size
    ...
    @staticmethod
    def backward(ctx, upstream_grad):
        grad_y_pred = (ctx.softmax_output - ctx.y_true) / ctx.batch_size
