...
class Tensor:
    ...
    def backward(self): 
        ...
        if self.data.size != 1:
            raise ValueError("backward() can only be called on a scalar (e.g., loss).")
        ...
        def build_topo(tensor):
            if tensor not in visited and tensor.ctx is not None:
                visited.add(tensor)
                for parent in tensor.ctx.inputs:
                    build_topo(parent)
                sorted_tensors.append(tensor)

        build_topo(self)

        for t in reversed(sorted_tensors):
            grad_fn = t.ctx.grad_fn
            inputs = t.ctx.inputs
            grads = grad_fn.backward(t.ctx, t.grad)

        
            if not isinstance(grads, tuple):
                grads = (grads,)

            for parent, grad in zip(inputs, grads):
